...
def generate_image(
    ...
    # Load models
    if model_config.model_name == 'flux':
        pipe = FluxPipeline.from_pretrained(model_config.model_path, torch_dtype=dtype)
        from snippets.flux import (
            forward_chords, forward_chords_worker,
        )
        from snippets.scheduler import custom_step_Euler as custom_step_flexible
    elif model_config.model_name == 'sd3-5':
        pipe = StableDiffusion3Pipeline.from_pretrained(model_config.model_path, torch_dtype=dtype)
        from snippets.sd3_5 import (
            forward_chords, forward_chords_worker,
        )
        from snippets.scheduler import custom_step_Euler as custom_step_flexible
    else:
    ...
    if rank > 0:
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            if algo_config.algo_name in ['chords'] and algo_config.full_return:
                image_list, latents_list = output.images
                for index, image in enumerate(image_list):
                    image_generate = image[0]
                    image_generate.save(f'{cur_output_path}/image_iter{index}.png')
                # save latents
                torch.save(latents_list, f'{cur_output_path}/latents.pt')
            else:
                image_generate = output.images[0]
                image_generate.save(f'{cur_output_path}/image.png')
